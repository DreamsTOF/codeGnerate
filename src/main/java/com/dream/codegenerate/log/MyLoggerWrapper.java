...
import com.dreamtof.log.core.LogLevel;
...
/**
 * 高性能日志装饰器
 * 核心逻辑：在将日志转发给 SLF4J 之前先进行权重预检，确保性能损耗最小化。
 */
public class MyLoggerWrapper implements MyLogger {

    private final Logger delegate;

    public MyLoggerWrapper(Logger delegate) {
        this.delegate = delegate;
    }

    // ========================================================================
    // 自定义级别核心实现 (基于 LogLevel 枚举)
    // ========================================================================

    @Override
    public boolean isLogLevelEnabled(LogLevel level) {
        if (level == null) return false;
        // 快速权重预检，映射到标准 SLF4J 级别检查
        if (level.weight >= LogLevel.ERROR.weight) return delegate.isErrorEnabled();
        if (level.weight >= LogLevel.WARN.weight) return delegate.isWarnEnabled();
        if (level.weight >= LogLevel.INFO.weight) return delegate.isInfoEnabled();
        if (level.weight >= LogLevel.DEBUG.weight) return delegate.isDebugEnabled();
    ...
    public void log(LogLevel level, String msg) {
        ...
        String out = "[" + level.name() + "] " + msg;
        if (level.weight >= LogLevel.ERROR.weight) {
            delegate.error(out);
        } else if (level.weight >= LogLevel.WARN.weight) {
            delegate.warn(out);
        } else {
            delegate.info(out);
        }
    ...
    public void log(LogLevel level, String msg, Throwable t) {
        if (!isLogLevelEnabled(level)) return;
        delegate.error("[" + level.name() + "] " + msg, t);
    }
    ...
    @Override public boolean isNoticeEnabled() { return isLogLevelEnabled(LogLevel.NOTICE); }
    @Override public void notice(String msg) { log(LogLevel.NOTICE, msg); }
    @Override public void notice(String format, Object arg) { log(LogLevel.NOTICE, format, arg); }
    @Override public void notice(String format, Object... args) { log(LogLevel.NOTICE, format, args); }
    @Override public void notice(String msg, Throwable t) { log(LogLevel.NOTICE, msg, t); }

    @Override public boolean isCriticalEnabled() { return isLogLevelEnabled(LogLevel.CRITICAL); }
    @Override public void critical(String msg) { log(LogLevel.CRITICAL, msg); }
    @Override public void critical(String format, Object arg) { log(LogLevel.CRITICAL, format, arg); }
    @Override public void critical(String format, Object... args) { log(LogLevel.CRITICAL, format, args); }
    @Override public void critical(String msg, Throwable t) { log(LogLevel.CRITICAL, msg, t); }

    @Override public boolean isFatalEnabled() { return isLogLevelEnabled(LogLevel.FATAL); }
    @Override public void fatal(String msg) { log(LogLevel.FATAL, msg); }
    @Override public void fatal(String format, Object arg) { log(LogLevel.FATAL, format, arg); }
    @Override public void fatal(String format, Object... args) { log(LogLevel.FATAL, format, args); }
    @Override public void fatal(String msg, Throwable t) { log(LogLevel.FATAL, msg, t); }
    ...
    @Override public void trace(Marker marker, String format, Object... argArray) { delegate.trace(marker, format, argArray); }
