...
public class MessageWindowChatMemory implements ChatMemory {
    ...
    private static void ensureCapacity(List<ChatMessage> messages, int maxMessages) {
        ...
        while (messages.size() > maxMessages) {
            ...
            // 根据你的规则，maxMessages 最小为6，且前3条消息受保护。
            // 因此，我们从索引 3 开始寻找第一个可以删除的目标。
            int messageToEvictIndex = -1; // 初始化为-1，表示还未找到

            for (int i = 3; i < messages.size(); i++) {
                ChatMessage currentMessage = messages.get(i);

                // 规则：我们只删除 AiMessage。如果不是，就跳过看下一个。
                if (currentMessage instanceof AiMessage) {
                    messageToEvictIndex = i; // 找到了！这就是我们要删除的目标
                    break; // 停止查找
                }
                // 如果是工具消息、用户消息等，则自动跳过，继续循环
            }

            // 如果在索引 3 之后找到了一个 AiMessage 进行删除
            if (messageToEvictIndex != -1) {
                // 1. 先删除这个 AiMessage
                ChatMessage evictedMessage = messages.remove(messageToEvictIndex);

                // 2. 级联删除：检查被删除的 AiMessage 是否有工具调用
                if (evictedMessage instanceof AiMessage aiMessage && aiMessage.hasToolExecutionRequests()) {
                    // 关键点：删除后，后面的元素会前移，所以我们要在同样的位置（messageToEvictIndex）
                    // 持续检查并删除紧跟其后的工具消息
                ...
                // 安全出口：如果在索引 3 之后完全找不到可以删除的 AiMessage，
                // 意味着剩下的都是受保护类型。为了避免无限循环，我们必须退出。
                break;
            }
        }
    }
